...
- [x] `npx playwright test tests/e2e/overrides.spec.ts` fails on the unlayered references and passes once they are wrapped, with the failure output recorded in the session notes or pull request.
- [x] Every `skills/ui-*/references/ui-*.md` css block opens with `@layer auk {` and `npx vitest run tests/objective.spec.ts` enforces it.
- [x] `node scripts/build-demos.mjs --check` exits 0 and each demo's `<style>` ends with its reference css byte for byte.
- [x] docs/properties.md lists every `--auk-*` property with its fallback and kind, and `npx vitest run tests/integration/properties-doc.spec.ts` fails when the file is edited by hand.
- [x] `npx vitest run tests/unit/ui-theme-mapping.spec.ts` passes with its counts unchanged while reading through scripts/auk-properties.mjs.
- [x] Each of the six references' Styles prose names the layer, the reset-before-`auk` condition, and the three places to set a property, wrapped at 88 columns.
- [x] docs/theming.md exists and README.md links both it and docs/properties.md.
- [x] No SKILL.md changes, and every SKILL.md body stays under 60 lines.
- [x] `npm run check` and `bash scripts/check.sh --prove` both exit 0.
