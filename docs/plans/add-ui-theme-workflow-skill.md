...
- Phase 1 settled (2026-09-03, corrected in review): `skillKind` whitelists the exact value `workflow` and returns `component` for everything else, because every caller tests `kind === 'component'` and a first draft that passed unknown strings through verbatim let a typo such as `kind: Workflow` exempt a skill from every strict gate; the review caught it, and tests/unit/skill-kind.spec.ts now pins it. A pure `kindOf(source)` sits beside it so malformed and missing frontmatter are unit-tested inline rather than with one fixture per case.
- Phase 1 settled: the component-only assertions in tests/objective.spec.ts are declared through `it.runIf(kind === 'component')`, so a workflow skill reports them as skipped in the run output rather than silently absent.
- Phase 1 settled: the `--prove` stanza asserts on the unit spec's exit status and the named fixture test, mirroring the frontmatter proof; the fixture is named `workflow-skill` without the `ui-` prefix because the prefix rule binds only paths under `skills/`.
- Phase 2 settled (2026-09-03): the reference's counts (69 brand-bearing, 57 colour, 7 radius, 5 font-family, 68 left alone) and every property in the mapping table are pinned to the component references by tests/unit/ui-theme-mapping.spec.ts, so the step 7 grep runs on every check rather than once - a new brand-bearing property in any component fails the gate until the table names it.
- Phase 2 settled: a role binds every property in its row, including those of components not yet built into the project - an unread custom property costs nothing and the next component built arrives themed; the worked example says so explicitly.
- Phase 3 settled: the palette test opens the dialog, opens a popover and populates the alert regions before scanning, because closed or empty surfaces paint none of the palette; every scan first asserts the palette reached `:root`, and the 1.4.11 test asserts the ring is the palette's own colour before computing its ratio against the nearest opaque ancestor, so neither can pass by measuring shipped colours.
- Phase 4 settled (2026-09-03): docs/plans/build-ui-component-skills.md gained the proposal link but its checked-in HTML was left as is - a fresh render differs from it by some two hundred lines of renderer-shell changes unrelated to this plan, and dragging those in would hide the one-sentence edit.
- Phase 4 settled: the README's opening line now says "Each component skill" and the layout tree says "one directory per skill", the two spots outside lines 18-23 and 40 that stopped being true with a workflow skill in the tree; the gates count in README, CLAUDE.md and the spec stays "six" because it counts gates.

## Steps

### Phase: Gate partition

1. [x] Create scripts/skill-kind.mjs exporting `skillKind(skillDir)` — read the directory's SKILL.md frontmatter with gray-matter and return `metadata.kind`, defaulting to `"component"` when the key or the file is absent or the frontmatter fails to parse, so a broken file always lands in the strict component gates. Why: three gates need one shared answer to "is this directory a component?", and a second definition would drift; it cannot live in scripts/build-demos.mjs because that file runs its main body on import. Verify: `node -e "import('./scripts/skill-kind.mjs').then(m=>console.log(m.skillKind('skills/ui-button')))"` prints `component`.
2. [x] Branch tests/objective.spec.ts on `skillKind`: a workflow skill keeps SKILL.md existence, the name prefix, frontmatter validation, body length, the Clarify heading, portability rules (over only the files that exist) and the evals assertions, and skips the component-only ones — the reference/demo file trio, react-demo shape, contract table, section order, Qualifiers line, css-var rules, demo-drift pins and WCAG rows (today's lines 50-54, 92-111, 121-161, 173-212, 214-236). Why: the suite is data-driven over skills/ and currently asserts every directory is a component, so any workflow skill fails on arrival. Verify: `npx vitest run tests/objective.spec.ts` stays green over the six component skills — the new branch is dormant until phase 2.
3. [x] Filter workflow skills out of tests/e2e/demos.spec.ts and scripts/build-demos.mjs using `skillKind`. Why: both walk skills/ expecting `references/demo.html`, and build-demos.mjs throws on a missing pair — one workflow skill would abort gate 4 for every skill at once. Verify: `node scripts/build-demos.mjs --check` prints "6 demo(s) up to date" and `npx playwright test tests/e2e/demos.spec.ts` still runs the six demo tests.
4. [x] Add the broken fixture tests/fixtures/workflow-skill/SKILL.md (valid frontmatter, `metadata.kind: workflow`, deliberately no evals file), a unit spec tests/unit/skill-kind.spec.ts (classifies the fixture as workflow, defaults skills/ui-button to component, and names the fixture in a rejection assertion), and a `--prove` stanza in scripts/check.sh mirroring the existing frontmatter one that greps the spec's verbose output for that fixture test. Why: the partition loosens a deliberately strict contract, and this repo's rule is that every gate must be provably fallible from tests/fixtures/. Verify: `bash scripts/check.sh --prove` prints the new ok line alongside every existing proof line.
5. [x] Add a short section to docs/component-spec.md defining a workflow skill: what `metadata.kind: workflow` means, which contract rules still bind it (naming, frontmatter, body rules, Clarify heading, portability, evals) and which do not apply (reference, demo, react-demo, WCAG table), and that such a skill stays public — it never sets `metadata.internal`. Why: the spec is the authoring contract and wins over CLAUDE.md, so an undocumented kind would make the partition folklore. Verify: the section reads back from the file and `npm run check` stays green.

### Phase: The ui-theme skill

6. [x] Author skills/ui-theme/SKILL.md: name `ui-theme`, license MIT, `metadata.kind: workflow`, a third-person description free of first- and second-person pronouns, and a body under 60 lines with no code fences carrying When to use, When not to use, a Clarify when needed heading (with the words description, props, requirements and ask) and a Build it list pointing at references/ui-theme.md. Why: SKILL.md is the installable surface agents discover, and the kept gates enforce exactly this shape. Verify: `npx vitest run tests/objective.spec.ts` shows ui-theme passing every kept assertion except the evals one, which step 9 lands.
