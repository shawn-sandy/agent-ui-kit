...
- Phase 1 settled (2026-09-03): `skillKind` returns any non-empty string kind verbatim and every caller compares it to `workflow`, so an unknown kind value faces the component gates; a pure `kindOf(source)` sits beside it so malformed and missing frontmatter are unit-tested inline rather than with one fixture per case.
