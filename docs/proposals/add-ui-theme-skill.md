...
> `docs/prompts/proposal-add-ui-theme-skill.md`. This copy is written for one deprecation
> release (plan-agent 6.0.0) and is removed in 6.1.0. Edit the prompt, not this file.

> This is a proposal for review, not an execution plan. It captures the measured token
> surface (137 properties across six references), the gate constraints that decide where a
> non-component skill can live, and the prior art on token extraction. The four load-bearing
> decisions are resolved (see Locked decisions); execution is handed off (see Next step).
...
- Recommended and resolved path: public workflow skill at `skills/ui-theme/`, static stylesheet
  discovery, brand-bearing scope, dark scheme mirrored only when the project already has one.
...
### A — Gate partition for workflow skills

Scope: let a `skills/*` directory declare `metadata.kind: workflow` and have the
component-only assertions skip it, while portability, frontmatter, evals and SKILL.md-body
rules still apply.
...
### B — The ui-theme skill

Scope: `skills/ui-theme/SKILL.md` (under 60 lines, no code fences) and
