...
| Reserved slug | `role` is never a component slug. `--auk-role-<name>` is the role layer, parsed under the same grammar, and `tests/objective.spec.ts` rejects a `ui-role` directory. | `--auk-role-primary` |
...
| How does a design-system team bind its tokens? | Through the role layer, never property by property. `skills/ui-theme/references/auk-roles.css` chains every brand-bearing property to one of 23 `--auk-role-*` properties inside the `auk` layer; a team copies it in and binds at most 23 lines, by hand or with `ui-theme`. `auk.tokens.json` beside it is the same contract as a DTCG token file. Both are generated by `scripts/build-tokens.mjs`; `docs/theming.md` is the guide. |
| Does a new brand-bearing property need a role? | Yes, exactly one: a row of the mapping table in `skills/ui-theme/references/ui-theme.md`. `tests/unit/ui-theme-mapping.spec.ts` fails until the table names it, and `tests/integration/tokens-file.spec.ts` pins the alias. A text colour never shares a role with the background it is drawn over - that is why `on-primary`, `on-muted` and `on-inverse` exist. |
| Does a measured property need a floor? | It carries a floor or `none` in `auk.tokens.json`, and a floor only after `tests/e2e/` asserts it - the map is in `scripts/build-tokens.mjs`, and the test checks the named spec has a test titled with the criterion. A number nobody measured never becomes a floor. |
| Do part, variant and state names have a design-tool form? | Yes, by rule and generated, never hand-kept: `data-part` values are Title Case layer names, variants are `Variant=<Value>`, states are `State=<Value>` with `State=Default` first. `docs/properties.md` renders the table per component from the qualifier line. Name a part, variant or state so both forms read well. |
| May a reference set `color-scheme`, ship a dark value or read a role? | No. `tests/objective.spec.ts` rejects `color-scheme`, `light-dark(` and `prefers-color-scheme` in a css block, and its `var()` rule already rejects a `--auk-role-*` read. Dark is a theme concern, mirrored from the project's own form by `ui-theme`. |
...
1. **Vitest** — `tests/objective.spec.ts` (frontmatter conforms, no vendor token,
   ...
   `tests/integration/tokens-file.spec.ts`, `properties-doc.spec.ts` and `design-sheet.spec.ts`
   (the generated files equal their generators).
...
- [ ] Every new brand-bearing property has a row in the ui-theme mapping table, and no role covers a text colour and its own background.
- [ ] Every new measured property's floor is `none`, or a bound a `tests/e2e/` test asserts, in `scripts/build-tokens.mjs`.
- [ ] Part, variant and state names read well as Figma layer, `Variant` and `State` names, and `docs/properties.md` was regenerated.
- [ ] The css block sets no `color-scheme`, uses no `light-dark()`, carries no `prefers-color-scheme` query and reads no `--auk-role-*` property.
