...
| `Props`, an id reference - typed `id reference`, or written `attribute="<id>"` | Generated inside the component when the referent is one of its own parts; a required prop when the referent lives outside it | `aria-labelledby` on the dialog is generated from its heading; `aria-controls` on a tab is generated from its panel; `data-dialog-open="<id>"` on an opener is a required prop of the opener |
