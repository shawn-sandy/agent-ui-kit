...
/** A SKILL.md at any depth. The walk returns root entries as bare names, so a leading slash cannot be assumed. */
const isSkillFile = (p: string) => basename(p) === 'SKILL.md';

const skills = readdirSync(ROOT, { recursive: true })
  .map((p) => String(p).replace(/\\/g, '/'))
  .filter((p) => isSkillFile(p) && !SKIP.test(p));
...
describe('the skills CLI sees exactly the packaged skills', () => {
  it('a SKILL.md at the repository root is scanned too', () => {
    expect(isSkillFile('SKILL.md')).toBe(true);
    expect(isSkillFile('skills/ui-button/SKILL.md')).toBe(true);
    expect(isSkillFile('docs/SKILL.md.bak')).toBe(false);
  });
